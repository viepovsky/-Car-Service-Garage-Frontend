    ...
    public List<AvailableCarRepairDto> getALlAvailableServices(Long garageId) {
        try {
            URI url = UriComponentsBuilder.fromHttpUrl(backendConfig.getAvailableCarServiceApiEndpoint() + "/" + garageId)
                    ...
                    .encode()
                    .toUri();
            AvailableCarRepairDto[] response = restTemplate.getForObject(url, AvailableCarRepairDto[].class);
            return Arrays.asList(ofNullable(response).orElse(new AvailableCarRepairDto[0]));
